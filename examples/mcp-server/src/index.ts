...
/**
 * Entry point for the railroute-ts MCP server. Speaks the Model Context Protocol
...
async function main(): Promise<void> {
  ...
  console.error('railroute-ts MCP server running on stdio');
}

main().catch((err) => {
  console.error('Fatal error starting railroute-ts MCP server:', err);
  process.exit(1);
});
